...
<!DOCTYPE html>
 ...
 <h3>Report : In-Patient Details (Transfers)</h3>

<form action="report_patient_details_tf_ip.php" method="post">
...
$unit=""; $i=0;
 ...
 $query= "SELECT
          	patients.patient_id \"pid\", visit_id, admit_id, hosp_file_no, name, mother_name, father_name, place, phone, gender, dob, admit_date, provisional_diagnosis
			FROM patient_visits
			INNER JOIN patients ON patient_visits.patient_id = patients.patient_id
			WHERE  ((admit_date BETWEEN '".$_POST['from_date']. "' AND '" . $_POST['to_date'] ."') AND (visit_id!=admit_id) AND(visit_type='IP')" . $dept . $unit . $area . $gender . ")";
 ...
 echo "<thead>";
 echo "<tr><th>S.no</th>";  echo "<th>Patient ID</th>";  echo "<th>Visit ID</th>";  echo "<th>Admit ID</th>"; echo "<th>IP NO</th>"; echo "<th>Patient Name</th>"; echo "<th>Mother's / Father's Name</th>"; echo "<th>Address/Phone</th>"; echo "<th>Sex</th>"; echo "<th>Age at Admission (days)</th>"; echo "<th>Provisional Diagnosis</th>"; echo "<th>Admit Date</th></tr>"; 
 echo "</thead>";
 echo "<tbody>";
